...
os.makedirs('data/graph/individual_graph/', exist_ok=True)

for pdb in tqdm(filenames, desc="Constructing Individual Graphs"):
    ...
    with torch.no_grad():
        for fasta in fasta1_list:
            ...
            input1 = {k: v.to(device) for k, v in input1.items()}  # 將輸入搬到 GPU
            ...
            output1_list.append(last_hidden_state1.cpu())  # 算完搬回 CPU 避免 VRAM 爆掉
