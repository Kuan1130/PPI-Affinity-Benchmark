...
def get_fasta_seq(pdb, directory):
    ...
    if pdb in mole2file:
        ...
        with open(directory + pdb + '_2.fasta', 'r') as f2:
            lines = f2.readlines()
            fasta_2 = "".join([line.strip() for line in lines[1:]]) # orginal: fasta_2 = lines[1].strip()
...
thred = 15
pdb_chain_tuple = get_chainlist_from_indexfile('data/chain_index.txt') 
# original: pdb_chain_tuple = get_chainlist_from_indexfile('data/index_example.txt')
...
for pdb, prot_pair in zip(pdblist, chainlist):
    ...
    residuelistA = get_residue_list_from_file('../../../../data/local/pdbqt/' + pdb + '_atom_processed' + '.pdbqt', prot_pair[0])
    residuelistB = get_residue_list_from_file('../../../../data/local/pdbqt/' + pdb + '_atom_processed' + '.pdbqt', prot_pair[1])
